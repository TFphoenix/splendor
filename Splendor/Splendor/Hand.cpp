...
Hand::Hand(std::vector<std::optional<ExpansionCard>> expansionCard, std::vector<std::optional<IToken>> token): m_expansionCard(expansionCard),m_token(token)
{
}

void Hand::AddToken(std::vector<std::optional<IToken>> token)
{
	if (m_token.size() <= 7)
		m_token == token;
