...
int main() {
	...
	// ** initialize winsock
	WSADATA winSocketData;	//the call to winsock startup will fill this; we need this for the initialization of the socket
	WORD version = MAKEWORD(2, 2);	//using version 2.2

	int winSocketOk = WSAStartup(version, &winSocketData);	//needs &, will be modified
	if (winSocketOk != 0) {
